...
if __name__ == '__main__':
    ...
    query = """create table newTable SELECT count(B.alpha) as alpha1,
                ...
                case when a =b then c when d=e then case when f=g then h else i end else g end as newCol,
                A.marsha as MARS,
    ...
    # u = p.queryObject.queryDict['select'][1]
    # print(u)
    print("where condition: ", p.whereClausePandasDF())
    for a in p.joinPandasDFs():
        print(a)
    # print(p.queryObject.tableAliases)
    # print(p.queryObject.tableColumnsDict)
    # print(p.whereClausePandasDF())from SQL_Query.SQLQuery import *
from Utils import *
import re
from moz_sql_parser import parse
